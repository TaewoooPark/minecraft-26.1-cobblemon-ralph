...
> ## 🔍 External Evaluation (2026-05-14)
>
> After the public release, an external reviewer surfaced two issues. Both are recorded here verbatim, with cause, evidence, and resolution status, so readers can judge the work without having to dig.
>
> ### Finding 1 — Reproduction failed: `RECONSTITUTE.md` returned HTTP 404
>
> - **Symptom**: Following `README §5` ("Reproduce from source") sent the reader to `impl/cobblemon-port/RECONSTITUTE.md`, which returned **HTTP 404** on `raw.githubusercontent.com`. The Option B (build-from-source) reproduction path was effectively broken; only Option A (jar download from the GitHub Release) worked.
> - **Cause**: The file was authored on **2026-05-13 18:46 — inside the solving window**, on the same day as the final PT191s build and `report.md v0.8`. The first public commit [`97b4a62`](https://github.com/TaewoooPark/minecraft-26.1-cobblemon-ralph/commit/97b4a62) already contained the line `!impl/cobblemon-port/RECONSTITUTE.md` in `.gitignore` — i.e. the publish intent was unambiguous from day one. However, git semantics forbid `!negation` from re-including a file under an excluded parent directory, so the line above (`impl/cobblemon-port/`) silently swallowed the exception. The file existed locally and was referenced from `README §5` and `RUN-MANUAL §4`, but never reached the remote.
> - **Resolution**: Commit [`cb96a1b`](https://github.com/TaewoooPark/minecraft-26.1-cobblemon-ralph/commit/cb96a1b) (2026-05-15) changed the gitignore syntax to `impl/cobblemon-port/*` so the negation works, then added the same **1717-byte file unchanged**. **No content was authored after the solving window closed.** Full evidence (file timestamp, original `.gitignore` line 14 negation, byte-identical content) is preserved in the [GitHub Release "Post-release errata" section](https://github.com/TaewoooPark/minecraft-26.1-cobblemon-ralph/releases/tag/v1.8.0%2B26.1.2-PT191s).
> - **Verdict**: Operational oversight, not a substantive solution gap.
>
> ### Finding 2 — Cobblemon UX coverage shortfall (genuine, unresolved)
>
> Full cause analysis and six proposed Ralph rule additions (Tier-1/Tier-2 acceptance split, R19 stub-debt ledger, R20 first-party data integrity, R21 deferred-work scheduling, U Update-scope slot, cost-weighted priority): **[`EVALUATION-FINDING-2.md`](EVALUATION-FINDING-2.md)**.
